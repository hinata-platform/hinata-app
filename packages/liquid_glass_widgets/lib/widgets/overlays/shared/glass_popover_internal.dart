      ...
      builder: (context, child) {
        ...
        return Stack(
          ...
          children: [
            ...
            //
            // LOCAL PATCH (hinata): target the ROOT overlay, not the nearest one.
            // The morph is placed with absolute, root-relative coordinates (the
            // trigger's `localToGlobal(Offset.zero)`), so it must render in the
            // overlay that shares that coordinate space. Rendering into a *nested*
            // overlay (e.g. a ShellRoute/nested-Navigator content area offset by a
            // side rail) shifts the popover by that overlay's origin — the
            ...
            // lands exactly on its trigger in every embedding. Re-applied after
            // upgrading the vendored package to 0.21.3 (upstream still targets the
            // nearest overlay).
            OverlayPortal(
