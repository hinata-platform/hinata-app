...
/// The octocat mark (GitHub's own path). GitLab / Bitbucket fall back to a
/// two-letter monogram, matching `app/git_panels.jsx`.
const String _githubMark =
...
/// A brand-coloured rounded tile for a provider — octocat for GitHub, a white
/// monogram (GL / BB) otherwise. Mirrors `.gp-glyph`.
class ProviderGlyph extends StatelessWidget {
  const ProviderGlyph({super.key, required this.provider, this.size = 26});
  ...
  Widget build(BuildContext context) {
    final radius = math.min(size * 0.3, 12).toDouble();
    return Container(
      width: size,
      height: size,
      ...
      child: provider == GitProvider.github
          ? SvgPicture.string(
              '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="$_githubMark"/></svg>',
              width: size * 0.62,
              height: size * 0.62,
