...
/// Applies a change to the server's cards, or refuses it.
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(planning.state.backlog, hasLength(12));
  ...
  test(
    ...
    () async {
      ...
      expect(planning.state.containerOf('s2').total, 4);
      expect(planning.state.backlog.map((c) => c.id), isNot(contains(card.id)));
      expect(server.reads.where((read) => read.sprintId == 's2'), hasLength(2));
    },
  );
  ...
  test(
    ...
    () async {
      ...
      final refusal = await planning.moveToSprint(
  ...
  test(
    ...
    () async {
      ...
      expect(server.reads.length, before, reason: 'nothing asked while typing');
  ...
  test(
    ...
    () async {
      ...
      expect(server.reads.length, before, reason: 'the head counts it already');
